function plotNoteSpectrum(y,xhatlms, xhatplms, xhatrls, fs)
...
for i = 1:length(noteidx)
    ...
    Pplms = pwelch(xhatplms(noteidx(i,1):noteidx(i,2)),blackman(N),N/2,1024,fs);
    ...
    plot(w, 10*log10(Pplms));
    plot(w, 10*log10(Prls));
    
    title(sprintf('Note %i', i));
    legend('Original', 'LMS', 'pLMS', 'RLS')
    xlabel('Frequency (Hz)')
    ylabel('Spectrum (dB)')
    hold off
end
